 <#
.SYNOPSIS
      Script to remediate STIG ID WN10-CC-000335: Disallow unencrypted WinRM client traffic
      Sets registry value AllowUnencryptedTraffic to 0
...
try {
    ...
    $registryValue = 0  # Disallow unencrypted traffic
    ...
    if ($currentValue.$registryName -eq $registryValue) {
        Write-Output "Successfully configured WinRM client to disallow unencrypted traffic."
    } else {
        throw "Failed to set 'AllowUnencryptedTraffic' to 0."
    }

} catch {
    Write-Error "An error occurred while remediating STIG WN10-CC-000335: $_"
    exit 1
}

Write-Output "STIG WN10-CC-000335 remediation completed successfully." 
